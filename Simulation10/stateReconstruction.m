...
% Normalize
psi = psi*1/sqrt(alpha^2+beta^2);
% Density matrix of Ideal State
rho_Ideal = psi*psi';
...
if simulation == 1
    for ii=1:nMeasurement
        ...
        countFactor=100;
        maxCount=1;
        [P_Noise{ii,1}, Sigma{ii,1}] = P_Noise_Poisson(P_Ideal, maxCount,countFactor);
        nonsignalingList{ii,1} = checkNonSignaling(P_Noise{ii,1});

        % Project on NS Polytop
        P_Noise{ii,2} = nonSignaling(P_Noise{ii,1});
        nonsignalingList{ii,2} = checkNonSignaling(P_Noise{ii,2});  


        % QST: Reconstruct Density Matrix
        [rho_Noise{ii,1}, M_Noise{ii,1}] = qst_linearinversion(Chi,P_QST_Selection(P_Noise{ii,1}));
        [rho_Noise{ii,2}, M_Noise{ii,2}] = qst_linearinversion(Chi,P_QST_Selection(P_Noise{ii,2}));
        ...
        toc
        tic;
        rho_Noise{ii,4} = qst_maximumlikelihood(rho_Noise{ii,2},Chi,P_QST_Selection(P_Noise{ii,2}));
        toc
        %tic;
        %rho_Noise{ii,5} = qst_maximumlikelihood(rho_Noise{ii,4},Chi,P_QST_Selection(P_Noise{ii,2}));
        %toc


        % Calculate Traces
        traceRhoList{ii,1} = trace(rho_Noise{ii,1});
        traceRhoList{ii,2} = trace(rho_Noise{ii,2});
        traceRhoList{ii,3} = trace(rho_Noise{ii,3});
        traceRhoList{ii,4} = trace(rho_Noise{ii,4});

        traceRho2List{ii,1} = trace(rho_Noise{ii,1}^2);
        traceRho2List{ii,2} = trace(rho_Noise{ii,2}^2);
        traceRho2List{ii,3} = trace(rho_Noise{ii,3}^2);
        traceRho2List{ii,4} = trace(rho_Noise{ii,4}^2);

        delta_P{ii,1}=P_Noise{ii,1}-P_Ideal;
        delta_P{ii,2}=P_Noise{ii,2}-P_Ideal;
        delta_P_Matrix(:,ii)=reshape(P_Noise{ii,1}-P_Ideal,36,1);
        delta_P_Matrix_Proj(:,ii)=reshape(P_Noise{ii,2}-P_Ideal,36,1);
...
if report==1
    ...
    nBins = max(abs(delta_P_Matrix(pIndex,:))')-min(abs(delta_P_Matrix(pIndex,:))')+1;
    histfit(abs(delta_P_Matrix(pIndex,:))'*countFactor,10,'poisson')
    pd = fitdist(abs(delta_P_Matrix(pIndex,:))','poisson');
    h=legend(['$\lambda_{fit}=',num2str(pd.lambda), '$ $\lambda_{target}=', num2str(lambda_Target),'$'])
    set(h,'Interpreter','latex')

    for jj=1:36 
        pd = fitdist(abs(delta_P_Matrix(jj,:))','poisson');
        pd_proj = fitdist(abs(delta_P_Matrix_Proj(jj,:))','poisson');
        var_fit(jj)=pd.lambda*countFactor;
        var_fit_proj(jj)=pd_proj.lambda;

        var_P(jj)=var(delta_P_Matrix(jj,:)*countFactor);
        var_P_proj(jj)=var(delta_P_Matrix_Proj(jj,:)*countFactor);
    end
