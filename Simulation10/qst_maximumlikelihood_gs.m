function [rho_reconstr, output] = qst_maximumlikelihood_gs(rhoLinearInversion,measurementBasis,countSignal)
...
L = @(t) real((measurementBasis{1}'*rho(t)*measurementBasis{1}-countSignal(1))^2 / 2*(measurementBasis{1}'*rho(t)*measurementBasis{1})...
    ...
    opt = optimoptions('lsqnonlin','MaxFunctionEvaluations', 80000, 'MaxIterations',80000,'UseParallel',1, 'Algorithm', 'levenberg-marquardt','OutputFcn',@outfun);
    ...
    function stop = outfun(optimvalues,state)
        stop=false;
        output{size(output,1)+1,1} =counter;
        output{size(output,1),2} =optimvalues.bestfval;
