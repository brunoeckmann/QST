...
timeElapsed=cell(nMeasurement,8);
...
for ii=1:nMeasurement
    ...
    tic;
    [rho_Noise{1}, M{1}] = qst_linearinversion(Chi,P_QST_Selection(P{1}));
    t(1) = toc;
    tic;
    [rho_Noise{2}, M{2}] = qst_linearinversion(Chi,P_QST_Selection(P{2}));
    t(2) = toc;

    % QST Maximum Likelihood lstsqr: Reconstruct Density Matrix
    tic;
    rho_Noise{3} = qst_maximumlikelihood(rho_Noise{2},Chi,P_QST_Selection(P{1}));
    t(3) = toc;
    tic;
    rho_Noise{4} = qst_maximumlikelihood(rho_Noise{2},Chi,P_QST_Selection(P{2}));
    t(4) = toc;

    % QST Maximum Likelihood fminsearch: Reconstruct Density Matrix
    tic;
    rho_Noise{5} = qst_maximumlikelihood_fmin(rho_Noise{2},Chi,P_QST_Selection(P{1}));
    t(5) = toc;
    tic;
    rho_Noise{6} = qst_maximumlikelihood_fmin(rho_Noise{2},Chi,P_QST_Selection(P{2}));
    t(6) = toc;

    % QST Maximum Likelihood GA: Reconstruct Density Matrix
    tic;
    rho_Noise{7} = qst_maximumlikelihood_ga(rho_Noise{2},Chi,P_QST_Selection(P{1}));
    t(7) = toc;
    tic;
    rho_Noise{8} = qst_maximumlikelihood_ga(rho_Noise{2},Chi,P_QST_Selection(P{2}));
    t(8) = toc;
    ...
    fprintf('\t QST Maximum Likelihood fmin for Noisy Behaviour in %f Seconds\n',t(5));
    fprintf('\t QST Maximum Likelihood fmin for Projected Behaviour in %f Seconds\n',t(6));
